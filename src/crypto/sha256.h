// sha256.h - SHA-256 hash implementation (FIPS 180-4)
...
class SHA256 {
public:
    static const size_t DIGEST_SIZE = 32; // 256 bits
    ...
    // Finalize and write 32-byte digest into out (caller-owned buffer of size >= 32)
    void finish(uint8_t out[32]) {
        uint64_t saved_bitlen = bitlen_;
        uint8_t pad = 0x80;
        update(&pad, 1);
        uint8_t zero = 0x00;
        ...
        uint8_t lenbytes[8];
        for (int i = 0; i < 8; ++i)
            lenbytes[i] = static_cast<uint8_t>(saved_bitlen >> (56 - 8 * i));
        // append length without touching bitlen accounting further
        std::memcpy(buffer_ + buflen_, lenbytes, 8);
        buflen_ += 8;
        transform(buffer_);
        buflen_ = 0;
        for (int i = 0; i < 8; ++i) {
